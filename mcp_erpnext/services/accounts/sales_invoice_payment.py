...
def _resolve_bank_account(
    invoice: Any, requested: str | None
...
def _resolve_mode_account(
    invoice: Any, mode: str | None
...
def _preview(doc: Any, source: Any, destination: str | None) -> dict[str, Any]:
    ...
    return {
        ...
        "source_status": (
            ...
            else str(_value(source, "status", "Unknown"))
        ),
...
def _fingerprint(
    request: dict[str, Any], preview: dict[str, Any], native_account: str | None
) -> str:
    material = {
        "request": request,
        "preview": preview,
        "resolved_destination_account": native_account,
    }
...
def _build(
...
) -> tuple[Any | None, Any | None, dict[str, Any] | None, str | None]:
    ...
    native_account, destination, failure = _resolve_bank_account(
        invoice, request.get("bank_account")
    )
    if failure:
        return None, None, failure, None
    if request.get("mode_of_payment"):
        native_account, failure = _resolve_mode_account(
            invoice, request["mode_of_payment"]
        )
        if failure:
            return None, None, failure, None
...
def prepare_sales_invoice_payment(request: dict[str, Any]) -> dict[str, Any]:
    ...
    preview = _preview(
        doc,
        invoice,
        normalized.get("mode_of_payment") or normalized.get("bank_account"),
    )
    token = approvals.create(
        ...
        payload={
            "request": normalized,
            "preview": preview,
            "fingerprint": _fingerprint(normalized, preview, native_account),
        },
    )
    return {
        "status": "ready",
...
def confirm_sales_invoice_payment(approval_token: str, confirm: bool) -> dict[str, Any]:
    ...
    return {
        "status": "created",
        "doctype": "Payment Entry",
        "payment_entry": _value(doc, "name"),
        "docstatus": 0,
        "source_sales_invoice": _value(invoice, "name"),
        "customer": _value(doc, "party"),
        "company": _value(doc, "company"),
        "idempotent": False,
    }
