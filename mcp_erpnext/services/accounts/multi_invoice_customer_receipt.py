...
def _destination(
    company: str, request: dict[str, Any]
    ...
    if bank:
        bank_doc, failure = _load(
            "Bank Account", str(bank), "INVALID_BANK_ACCOUNT", "Bank Account"
        )
        if failure:
            return None, failure
        if _value(bank_doc, "company") not in (None, "", company) or not _value(
            bank_doc, "account"
        ):
            return None, _error(
        ...
        for account_type in ("Bank", "Cash"):
            resolved = get_default_bank_cash_account(
                company, account_type, mode_of_payment=mode, fetch_balance=False
            )
    ...
    return {
        ...
        "identity": identity,
        "account": account,
        "currency": str(details.account_currency),
        "account_type": _value(details, "account_type"),
    }, None
...
def _is_early_discount_eligible(
    invoice: Any, posting_date: Any, party_currency: str
) -> bool:
    from erpnext.accounts.doctype.payment_entry.payment_entry import (
        apply_early_payment_discount,
    )

    _, _, discount, _ = apply_early_payment_discount(
        0, 0, invoice, party_currency, getdate(posting_date)
    )
...
def _reference_preview(row: Any, invoice: Any, party_currency: str) -> dict[str, Any]:
    return {
        ...
        "posting_date": _json(
            _value(row, "posting_date") or _value(invoice, "posting_date")
        ),
    ...
    return [
        {
            ...
            "amount": flt(_value(row, "amount")),
            "description": _value(row, "description"),
        }
        for row in (_value(doc, "deductions", []) or [])
    ]


def _preview(
    doc: Any, invoices: dict[str, Any], destination: dict[str, Any]
) -> dict[str, Any]:
    party_currency = str(_value(doc, "paid_from_account_currency"))
    references = [
        _reference_preview(
            row, invoices[str(_value(row, "reference_name"))], party_currency
        )
...
def _fingerprint(
    ...
    for name in sorted(invoices):
        ...
        sources.append(
            {
                "name": name,
                "docstatus": int(_value(invoice, "docstatus", 0) or 0),
                "modified": str(_value(invoice, "modified")),
                "customer": _value(invoice, "customer"),
                "company": _value(invoice, "company"),
                "account": _effective_receivable(invoice),
                "currency": _value(invoice, "currency"),
                "party_currency": _value(invoice, "party_account_currency"),
                "posting_date": _json(_value(invoice, "posting_date")),
                "due_date": _json(_value(invoice, "due_date")),
                "grand_total": _value(invoice, "grand_total"),
                "outstanding": by_name[name]["outstanding_amount"],
                "allocated": by_name[name]["allocated_amount"],
                "payment_terms_template": _value(invoice, "payment_terms_template"),
...
def _build(
    ...
    Any | None, dict[str, Any] | None, dict[str, Any] | None, dict[str, Any] | None
]:
    ...
    invoices: dict[str, Any] = {}
    for name in names:
        invoice, failure = _load(
            "Sales Invoice", name, "SOURCE_NOT_FOUND", f"Sales Invoice {name}"
        )
        if failure:
            return None, None, None, failure
        invoices[name] = invoice
    if any(
        int(_value(invoice, "docstatus", 0) or 0) != 1 for invoice in invoices.values()
    ):
        return (
            None,
            None,
    ...
    if any(
        _value(invoice, "customer") != _value(customer, "name")
    ...
    party_currencies = {
        _value(invoice, "party_account_currency") for invoice in invoices.values()
    }
    if len(companies) != 1:
        return (
            None,
    ...
    company, account, party_currency = (
        str(next(iter(companies))),
        str(next(iter(accounts))),
        str(next(iter(party_currencies))),
    )
    ...
    if any(
        _is_early_discount_eligible(invoice, request["posting_date"], party_currency)
        for invoice in invoices.values()
    ):
        return (
            None,
        ...
        get_outstanding_reference_documents(
            {
                ...
                "party_type": "Customer",
                "payment_type": "Receive",
                "party": str(_value(customer, "name")),
                "party_account": account,
                "get_outstanding_invoices": True,
                "get_orders_to_be_billed": False,
                "vouchers": [
                    frappe._dict(voucher_type="Sales Invoice", voucher_no=name)
                    for name in names
                ],
                "book_advance_payments_in_separate_party_account": bool(
                    frappe.db.get_value(
                        "Company",
                        company,
    ...
    for row in rows:
        if (
            _value(row, "voucher_type") == "Sales Invoice"
            and _value(row, "voucher_no") in invoices
        ):
    ...
    requested = {
        str(row["sales_invoice"]): flt(row["allocated_amount"], precision)
    ...
    if any(
        value <= 0
        or value > flt(_value(by_name[name], "outstanding_amount"), precision)
        ...
        doc.update(
            {
                ...
                "paid_amount": amount,
                "received_amount": (
                    flt(request.get("bank_amount"))
                    if request.get("bank_amount") is not None
                    else amount
                ),
        ...
        for name in sorted(names):
            ...
            doc.append(
                "references",
                {
                    ...
                    "reference_name": name,
                    "total_amount": _value(native, "invoice_amount"),
                    "outstanding_amount": _value(native, "outstanding_amount"),
                    "allocated_amount": requested[name],
                    "due_date": _value(native, "due_date"),
                    "exchange_rate": _value(native, "exchange_rate"),
                    "payment_term": None,
                },
            )
...
def prepare_multi_invoice_customer_receipt(request: dict[str, Any]) -> dict[str, Any]:
    ...
    token = approvals.create(
        ...
        payload={
            "request": normalized,
            "preview": preview,
            "fingerprint": _fingerprint(normalized, preview, invoices, destination),
        },
    )
    return {
        "status": "ready",
...
def confirm_multi_invoice_customer_receipt(
    ...
    return {
        ...
        "customer": str(_value(doc, "party")),
        "company": str(_value(doc, "company")),
        "source_sales_invoices": sorted(invoices),
        "idempotent": False,
    }
