...
def _load_source(name: str) -> tuple[Any | None, dict[str, Any] | None]:
    try:
        ...
        return None, _error("SOURCE_NOT_FOUND", "The requested Quotation was not found.")
    if not quotation.has_permission("read"):
        return None, _error("PERMISSION_DENIED", "The authenticated user cannot read that Quotation.")
    ...
    if not quotation.get("party_name"):
        return None, _error("SOURCE_NOT_READY", "The Submitted Quotation has no Customer party.")
...
def _fingerprint(quotation: Any, preview: dict[str, Any], mapped_doc: Any) -> str:
    ...
    snapshot = {
        ...
        "sales_order": {
            **{key: sales_order.get(key) for key in (
                "customer",
                "customer_name",
                "company",
                "transaction_date",
...
def _map_and_preview(quotation: Any) -> tuple[Any | None, dict[str, Any] | None, dict[str, Any] | None]:
    ...
    if int(sales_order.docstatus or 0) != 0:
        return None, None, _error("CONVERSION_UNAVAILABLE", "The mapped Sales Order is not a Draft.")
...
def prepare_quotation_to_sales_order(quotation: str) -> dict[str, Any]:
    ...
    user = _current_user()
    source, failure = _load_source(quotation.strip() if isinstance(quotation, str) else "")
    if failure:
        return failure
...
def confirm_quotation_to_sales_order(approval_token: str, confirm: bool) -> dict[str, Any]:
    ...
    try:
        sales_order.insert(ignore_permissions=False, ignore_links=False, ignore_mandatory=False)
        frappe.db.commit()
    except frappe.PermissionError:
        frappe.db.rollback()
        return _error("PERMISSION_DENIED", "The authenticated user cannot create the converted Sales Order.")
    except Exception:
        frappe.db.rollback()
        return _error(
            "CONVERSION_FAILED",
