...
_COMMERCIAL_DEFAULTS = ("selling_price_list", "price_list_currency", "currency", "conversion_rate", "plc_conversion_rate")


def _current_user() -> str:
	user = getattr(frappe.session, "user", None)
	if not user or user in {"Guest", "guest"}:
		frappe.throw("An authenticated Frappe user is required for Quotation creation.", frappe.PermissionError)
	return user
...
def _permitted_record(doctype: str, name: str, fields: list[str], filters: dict[str, Any] | None = None) -> dict[str, Any] | None:
...
def _resolve_company(company: Any) -> str | None:
	if company is not None and (not isinstance(company, str) or not company.strip()):
		return None
	resolved = company.strip() if isinstance(company, str) else frappe.defaults.get_user_default("Company")
	if not resolved:
		return None
...
def _permitted_link(
	...
	if not isinstance(value, str) or not value.strip():
		return None, _error("INVALID_QUOTATION_DETAILS", f"{label} must be a non-empty name.")
	name = value.strip()
	if not _permitted_record(doctype, name, [], filters):
		return None, _error("INVALID_QUOTATION_DETAILS", f"{label} is not available to the authenticated user.")
	return name, None
...
def _number(value: Any, *, field: str, positive: bool = False) -> tuple[float | None, dict[str, Any] | None]:
	...
	if not math.isfinite(number) or (positive and number <= 0) or (not positive and number < 0):
		...
		return None, _error("INVALID_QUOTATION_DETAILS", f"{field} must be {constraint}.")
	return number, None
...
def _date(value: Any, *, field: str) -> tuple[Any | None, dict[str, Any] | None]:
	...
	try:
		...
		return None, _error("INVALID_QUOTATION_DETAILS", f"{field} must be a valid date.")
...
def _prepare_items(items: Any) -> tuple[list[dict[str, Any]] | None, dict[str, Any] | None]:
	...
	for index, raw in enumerate(items, start=1):
		if not isinstance(raw, dict):
			return None, _error("INVALID_QUOTATION_DETAILS", f"Item row {index} must be an object.")
		item_name = _reference_name(raw.get("item"), "Item")
		if not item_name:
			return None, _error("INVALID_QUOTATION_DETAILS", f"Item row {index} requires a resolved Item reference.")
		item = _permitted_record("Item", item_name, ["item_code", "item_name", "stock_uom"], {"disabled": ["!=", 1], "is_sales_item": 1})
		if not item:
			return None, _error("INVALID_ITEM", f"Item row {index} is not available to the authenticated user.")
		quantity, failure = _number(raw.get("qty"), field=f"Item row {index} quantity", positive=True)
		if failure:
			return None, failure
		row = {"item_code": item["name"], "qty": quantity}
		for fieldname, label in (("rate", "rate"), ("discount_percentage", "discount percentage"), ("discount_amount", "discount amount")):
...
def _preview(doc: Any) -> dict[str, Any]:
	return {
		"customer": {"doctype": "Customer", "name": doc.party_name, "customer_name": doc.customer_name},
		"company": doc.company,
...
def prepare_quotation(
	...
	if not _permitted_record("Customer", customer_name, ["customer_name"], {"disabled": ["!=", 1]}):
		return _error("INVALID_CUSTOMER", "Customer is not available to the authenticated user.")
	...
	if failure:
		return failure
	resolved_terms, failure = _permitted_link("Terms and Conditions", tc_name, label="Terms and conditions")
	if failure:
		return failure
	if additional_discount_percentage is not None and discount_amount is not None:
		return _error("INVALID_QUOTATION_DETAILS", "Provide either a discount percentage or discount amount, not both.")
	additional_discount = None
	if additional_discount_percentage is not None:
		additional_discount, failure = _number(additional_discount_percentage, field="Additional discount percentage")
		if failure:
			return failure
