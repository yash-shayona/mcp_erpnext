...
def _load(doctype: str, name: str, profile: str) -> tuple[Any, dict[str, Any] | None]:
    ...
    for permission in ("read", "email", "print"):
        if not doc.has_permission(permission):
            return None, _error(
                "PERMISSION_DENIED",
                f"The authenticated user cannot {permission} that document.",
            )
    return doc, None
...
def _resolve_recipient(
    ...
    if not party_type or not party_name:
        return (
            ...
            _error(
                "RECIPIENT_NOT_FOUND",
                "The document has no associated business party email.",
            ),
        )
    ...
    if native_contact_email:
        candidate = _candidate(
            native_contact_email,
            doc.get("contact_display") or doc.get("contact_person"),
        )
        if not candidate:
            return (
                None,
                ...
                _error(
                    "INVALID_EMAIL",
                    "The document's associated contact email is invalid.",
                ),
            )
        ...
        if selected_contact:
            selected_candidates = _contact_candidates(
                party_type, party_name, selected_contact
            )
            candidates = selected_candidates or (
                [party_candidate]
                if party_candidate
                else _contact_candidates(party_type, party_name)
            )
        else:
            candidates = (
                [party_candidate]
    ...
    if requested is not None:
        ...
        if not requested_candidate:
            return (
                ...
                _error(
                    "INVALID_EMAIL",
                    "recipient_email must contain one valid email address.",
                ),
            )
        if not any(
            item["email"].casefold() == requested_candidate["email"].casefold()
            for item in candidates
        ):
            return (
                None,
                ...
                _error(
                    "INVALID_RECIPIENT",
                    "The requested recipient is not associated with this document's business party.",
                ),
            )
    ...
    return (
        ...
        _error(
            "RECIPIENT_NOT_FOUND",
            "No valid email recipient is associated with this document's business party.",
        ),
    )
    ...
    try:
        ...
        if not EmailAccount.find_outgoing(match_by_doctype=doctype, _raise_error=True):
            return _error(
                ...
                "No outgoing Frappe Email Account is configured.",
            )
        ...
        return _error(
            ...
            "Frappe could not validate the outgoing Email Account.",
        )
    return None
...
def _render(
    ...
    result = pdf_service.render_document_pdf(
    ...
    if result.get("status") != "ok":
        return None, _error(
            result.get("code", "PDF_RENDER_FAILED"),
            result.get("message", "Frappe could not render the PDF."),
        )
    return result, None
...
def confirm_document_email(approval_token: str, profile: str) -> dict[str, Any]:
    ...
    if failure:
        return failure
    if (
        str(doc.modified) != payload["modified"]
        or int(doc.docstatus) != payload["docstatus"]
    ):
        return _error(
    ...
    if (
        ...
        or recipient["email"].casefold() != payload["recipient_email"].casefold()
    ):
        return _error(
            "PREPARED_STATE_CHANGED",
            "The approved recipient is no longer associated with the document. Prepare it again.",
        )
    pdf, pdf_failure = _render(
        payload["doctype"],
        payload["name"],
    ...
    if (
        ...
        return _error(
            "PREPARED_STATE_CHANGED",
            "The approved PDF changed after the email preview. Prepare it again.",
        )
    ...
    try:
        ...
        frappe.db.rollback()
        return _error(
            "PERMISSION_DENIED", "The authenticated user cannot queue this email."
        )
    except Exception:
        frappe.db.rollback()
        return _error(
            "EMAIL_QUEUE_FAILED", "Frappe could not queue the document email."
        )
    return {
        "status": "queued",
        "doctype": payload["doctype"],
