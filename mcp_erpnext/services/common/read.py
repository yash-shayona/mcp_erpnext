...
_DOCUMENTS = {
	"Sales Order": {
		...
		"fields": ("name", "customer", "transaction_date", "delivery_date", "docstatus", "status", "currency", "grand_total"),
		"child_table": "items",
	},
	"Quotation": {
		"party_field": "party_name",
		"secondary_field": "valid_till",
		"fields": ("name", "party_name", "quotation_to", "transaction_date", "valid_till", "docstatus", "status", "currency", "grand_total"),
		"child_table": "items",
	},
	"Purchase Order": {
		"party_field": "supplier",
		"secondary_field": "schedule_date",
		"fields": ("name", "supplier", "transaction_date", "schedule_date", "docstatus", "status", "currency", "grand_total"),
		"child_table": "items",
	},
}
...
def _summary(doc: Any, doctype: str, *, include_items: bool = False) -> dict[str, Any]:
	...
	if include_items:
		result["items"] = [
			{field: row.get(field) for field in _ITEM_FIELDS if row.get(field) is not None}
...
def _filters(doctype: str, criteria: dict[str, Any]) -> dict[str, Any]:
	...
	if criteria.get("date_from"):
		filters["transaction_date"] = [">=", _normalise_dates(criteria["date_from"])]
	if criteria.get("date_to"):
		if criteria.get("date_from"):
			filters["transaction_date"] = ["between", [_normalise_dates(criteria["date_from"]), _normalise_dates(criteria["date_to"])]]
		else:
			filters["transaction_date"] = ["<=", _normalise_dates(criteria["date_to"])]
	return filters
...
def get_document(target: dict[str, str], profile: str) -> dict[str, Any]:
	...
	if not doc.has_permission("read"):
		return _error("PERMISSION_DENIED", "The authenticated user cannot read that document.")
	return {"status": "ok", "document": _summary(doc, doctype, include_items=True)}


def search_documents(doctype: str, criteria: dict[str, Any], profile: str) -> dict[str, Any]:
	if doctype not in _DOCUMENTS or doctype not in _profile_doctypes(profile):
		return _error("DOCTYPE_NOT_ALLOWED", f"{doctype} is not available in the {profile} MCP profile.")
...
def _profile_doctypes(profile: str) -> frozenset[str]:
	return {
		"sales": frozenset({"Quotation", "Sales Order"}),
