"""Permission-aware, allowlisted Item read and query services."""
...
_SORT_FIELDS = frozenset(
    {"name", "item_code", "item_name", "item_group", "disabled", "creation", "modified"}
)
...
def _filters(criteria: dict[str, Any]) -> list[list[Any]]:
    ...
    for field in ("disabled", "is_sales_item", "is_purchase_item", "is_stock_item"):
        if criteria.get(field) is not None:
...
def _requested_fields(requested: list[str]) -> list[str]:
    if any(field not in _FIELDS for field in requested):
        raise ValueError("Item projection contains an unsupported field.")
    return requested


def _sort_expression(sort_by: str, sort_order: str) -> str:
    if sort_by not in _SORT_FIELDS or sort_order not in {"asc", "desc"}:
        raise ValueError("Item sort contains an unsupported field or order.")
...
def get_item(item: str, fields: list[str]) -> dict[str, Any]:
    ...
    if not doc.has_permission("read"):
        return _error("PERMISSION_DENIED", "The authenticated user cannot read that Item.")
    return {"status": "ok", "document": _project(doc, fields)}


def query_items(criteria: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "status": "ok",
        "items": [_project(row, fields) for row in rows],
        "count": len(rows),
...
def aggregate_items(criteria: dict[str, Any]) -> dict[str, Any]:
