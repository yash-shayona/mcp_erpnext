...
After completing this task, do not return the detailed completion report only in chat.

Inspect the final implemented source and create a permanent implementation/inspection report at:

mcp_erpnext/docs/inspect/GENERIC_DOCUMENT_PDF_FOUNDATION_IMPLEMENTATION_REPORT.md

This file must be created even if the implementation is partially blocked or some tests fail.
...
List the exact current source files inspected in:

- mcp_erpnext
- Frappe v16 print/PDF implementation
...
Document the exact implemented public tool name and schema.

Example only:

render_document_pdf

Do not describe the planned contract if the implementation differs.
Document the actual final contract.

## 5. Frappe-Native APIs Used

Document the exact functions/modules reused for:

- Print Format resolution
- PDF generation
- Letter Head handling
- permissions

Also explain why these native APIs were chosen.

## 6. Print Format Resolution

Document the actual observed resolution order, including:

- explicit print format
- configured/default print format
- Standard fallback

Do not guess. Confirm against installed Frappe source/runtime.

## 7. Profile Policy

Document the actual server-side allowed DocTypes for each MCP profile.
...
Use the actual implemented configuration.

## 8. Permission Behavior

Explain how the authenticated Frappe user is preserved and how:
...
Explicitly confirm that Administrator/service-user/ignore-permissions bypass is not used.

## 9. PDF Artifact Behavior

Explain exactly how the generated PDF is returned through MCP:
...
- whether a Frappe File document is created

## 10. Runtime Test Matrix

| Test | Expected | Observed | Result |
|---|---|---|---|
| Sales Order default PDF | valid PDF | ... | PASS/FAIL |
| Quotation default PDF | valid PDF | ... | ... |
| Purchase Order default PDF | valid PDF | ... | ... |
| Explicit valid Print Format | works | ... | ... |
| Invalid Print Format | rejected | ... | ... |
| Cross-profile DocType | rejected | ... | ... |
| Missing document | not found | ... | ... |
| No read permission | denied | ... | ... |
| No print permission | denied | ... | ... |
| Letter Head/default behavior | Frappe-native | ... | ... |

Never fabricate Observed values.

## 11. End-to-End MCP / LibreChat Verification

Document whether these prompts were actually tested:

- "Give me the PDF of Sales Order SAL-ORD-XXXX."
- "Give me the PDF of Quotation QTN-XXXX."
- "Give me the PDF of Purchase Order PUR-ORD-XXXX."

Also document which MCP profile/server handled each request.

If LibreChat was not tested, explicitly write:

NOT VERIFIED IN LIBRECHAT

## 12. Regression Results

Confirm whether existing:

- sales tools
- purchase tools
- Sales Order read/query/analytics
...
- stale documentation
- unexpected Frappe behavior
- MCP SDK limitation
- LibreChat artifact-display limitation
- permission issue
- profile configuration issue

## 14. Remaining Limitations

List only actual known limitations.
...
The email implementation must reuse the generic PDF service created by this task.